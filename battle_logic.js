...
window.calculateDamageMultiplier = (atkRgb, defRgb) => {
...
window.createBattleEngine = (canvas, myParts, enemyParts, onEnd, timeState, drawSmoothPath) => {
    const { setTimeLeft, isEnded, timeLeftRef, timerAccumulator } = timeState;
    ...
    const GRAVITY = 800;
    const FLOOR_Y = 450;
    const camera = { x: 1600, y: 300, scale: 0.5 };
    ...
    const loop = (time) => {
        ...
        camera.scale += (targetScale - camera.scale) * dt * 5;
